...
const FormSectionHeader = ({ icon: Icon, title, note }) => (
  <div className="flex flex-col md:flex-row md:items-end justify-between gap-3 text-[#000000]">
    <div className="flex items-center gap-3">
      <div className="w-1.5 h-8 bg-[#000000] rounded-full"></div>
...
const LogoUpload = ({ preview, readOnly, onUploadClick, onFileChange, inputRef }) => (
  <div className="w-full lg:w-[320px] shrink-0 flex flex-col items-center gap-6">
    ...
    <div 
      ...
      className={`w-full aspect-square bg-gray-50 rounded-[40px] border-2 border-dashed border-gray-100 flex flex-col items-center justify-center text-center p-8 transition-all relative overflow-hidden group ${!readOnly ? "cursor-pointer hover:bg-gray-100 hover:border-[#000000]/20" : ""}`}
    >
      {preview ? (
        <div className="w-full h-full relative group">
          ...
          {!readOnly && (
            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
              <div className="bg-white p-3 rounded-full text-[#000000] shadow-lg">
        ...
        <div className="space-y-4 flex flex-col items-center">
          <div className="w-20 h-20 bg-white rounded-3xl flex items-center justify-center text-gray-300 shadow-sm group-hover:text-[#000000] transition-colors">
    ...
    {!readOnly && preview && (
      <button 
        ...
        className="flex items-center gap-2 text-[#000000] font-bold text-sm hover:underline"
      >
        ...
        <div className="flex items-center gap-3">
          {mode === "view" && (
            <button
              ...
              className="flex items-center gap-2 px-6 py-2.5 bg-[#000000] text-white rounded-2xl font-bold hover:bg-[#1f4523] transition-all shadow-lg shadow-[#000000]/20 text-sm"
            >
              <Pencil className="w-4 h-4" />
              Chỉnh sửa hồ sơ
            </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-6">
                ...
                <Controller 
                  ...
                  rules={{ 
                    ...
                    pattern: {
                      value: /^[0-9]{10}(-[0-9]{3})?$/,
                      message: "Mã số thuế không đúng định dạng (ví dụ: 0123456789 hoặc 0123456789-001)"
                    }
                  }}
                  render={({ field }) => (
                    <InputField 
                  ...
                  rules={{
                    validate: (value) => {
                      ...
                      try {
                        ...
                        return (url.protocol === "http:" || url.protocol === "https:") || "Website phải bắt đầu bằng http:// hoặc https://";
                      } catch {
                        return "URL website không hợp lệ (ví dụ: https://company.com)";
                      }
                    }
                  }}
                  render={({ field }) => (
                ...
                <Controller 
                  ...
                  rules={{
                    validate: (value) => {
                      ...
                      if (date > new Date()) return "Ngày thành lập không được là ngày trong tương lai";
                      return true;
                    }
                  }}
                  render={({ field }) => (
          ...
          <div className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {companyAddresses.map((addr) => (
                <div 
                  key={addr.id} 
                  className="flex items-start gap-4 p-5 bg-gray-50 border border-gray-100 rounded-3xl relative group hover:border-[#000000]/30 hover:bg-white transition-all shadow-sm"
                >
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${addr.isPrimary ? "bg-[#000000] text-white" : "bg-white text-[#000000] border border-gray-100"}`}>
              ...
              {!readOnly && !showAddAddressForm && (
                <button 
                  ...
                  className="flex flex-col items-center justify-center gap-3 p-8 border-2 border-dashed border-gray-100 rounded-3xl text-gray-400 hover:text-[#000000] hover:border-[#000000]/20 hover:bg-gray-50 transition-all group min-h-[140px]"
                >
            ...
            {showAddAddressForm && (
              <div className="bg-gray-50/50 border border-gray-100 rounded-[40px] p-8 md:p-10 space-y-8 animate-in fade-in slide-in-from-top-4 duration-300">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-1.5 h-6 bg-[#000000] rounded-full"></div>
                    ...
                    <div className="flex gap-3 max-w-2xl">
                      {["Trụ sở chính", "Chi nhánh", "Văn phòng đại diện"].map((type) => (
                        <button
                          ...
                          className={`flex-1 py-3 px-4 rounded-2xl text-xs font-bold transition-all border ${tempAddress.addressType === type ? "bg-[#000000] text-white border-[#000000] shadow-lg shadow-[#000000]/20" : "bg-white text-gray-500 border-gray-100 hover:bg-gray-50"}`}
                        >
                          {type}
                        </button>
                      ))}
                ...
                <div className="flex justify-end gap-4 pt-4">
                  ...
                  <button 
                    ...
                    className="px-10 py-3 bg-[#000000] text-white rounded-2xl font-bold shadow-lg shadow-[#000000]/10 hover:bg-[#1f4523] transition-all text-sm"
                  >
                    Xác nhận thêm
                  </button>
                </div>
              ...
              {!readOnly && (
                <div className="lg:col-span-3">
                  <div 
                    ...
                    className="w-full aspect-square bg-gray-50 rounded-[40px] border-2 border-dashed border-gray-100 flex flex-col items-center justify-center text-center p-8 cursor-pointer hover:bg-gray-100 hover:border-[#000000]/20 transition-all group"
                  >
                    <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center text-[#000000] mb-4 shadow-sm group-hover:scale-110 transition-transform">
              ...
              <div className={`${readOnly ? "lg:col-span-12" : "lg:col-span-9"} space-y-4`}>
                {docFiles.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {docFiles.map((file) => (
                      <div key={file.id} className="flex items-center gap-5 p-5 bg-white border border-gray-100 rounded-[24px] group hover:border-[#000000]/20 hover:shadow-md hover:shadow-gray-100 transition-all">
                        <div className="w-14 h-14 bg-red-50 rounded-2xl flex items-center justify-center text-red-500 shrink-0">
                          <File className="w-7 h-7" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="font-bold text-gray-900 truncate group-hover:text-[#000000] transition-colors">{file.name}</p>
                        ...
                        <div className="flex items-center gap-2">
                          <button 
                            ...
                            title="Xem tài liệu"
                            className="p-2.5 text-gray-400 hover:text-[#000000] hover:bg-gray-50 rounded-xl transition-all"
                          >
        ...
        <div className="flex flex-col sm:flex-row justify-end items-center gap-4 sm:gap-6 pt-10 border-t border-gray-100">
          ...
          {!readOnly && (
            <button 
              type="submit" 
              disabled={loading} 
              className="w-full sm:w-auto px-12 py-3.5 bg-[#000000] text-white rounded-[20px] font-bold shadow-xl shadow-[#000000]/20 hover:bg-[#1f4523] hover:-translate-y-0.5 transition-all active:translate-y-0 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
                <div className="flex items-center gap-2">
